  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="port-card p-6 sm:p-10 bg-gradient-to-r from-[#04172F] to-[#0a2a54] text-white rounded-[2.5rem] shadow-2xl border border-white/5 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-400/10 rounded-full -mr-32 -mt-32 blur-3xl pointer-events-none" />
        
        <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-10 gap-4">
          <div>
            <h3 className="text-2xl sm:text-3xl font-display font-black mb-2 tracking-tight">Service Provider Journey</h3>
            <p className="text-sm opacity-60 max-w-xl">Accelerated revenue recognition through real-time service logging and automated invoicing.</p>
          </div>
        </div>
            ...
            return (
              <div key={step.id} className="flex-1 flex flex-row md:flex-col items-center gap-4 md:gap-0 md:text-center group relative">
                <div className={`w-14 h-14 sm:w-16 sm:h-16 rounded-[1.25rem] flex items-center justify-center shrink-0 transition-all ${
                  step.status === 'active' 
                    ? 'bg-emerald-400 text-[#04172F] shadow-lg shadow-emerald-400/20 scale-110 md:mb-4' 
                    : 'bg-white/10 text-white/40 md:mb-4'
                }`}>
                  <StepIcon className="text-xl sm:text-2xl" />
                </div>
                <div className="flex-1 md:flex-none">
                  <p className={`text-[10px] font-black uppercase tracking-[0.2em] ${step.status === 'active' ? 'text-emerald-400' : 'text-white/80'}`}>{step.title}</p>
